...
namespace AutoTest.Core.Helpers
{
    public static class StringExtensions
    {
        ...
        public static string AddNewContext(this string prefix, string content, Symbols symbol = Symbols.Braces)
        {
            ...
            stringBuilder.Append(isSpaceRequired ? content.AddIdentation() : content);
        ...
        public static string AddIdentation(this string text, int identationLevel = 1)
            => $"{Identation.Repeat(identationLevel)}{text.Replace(Environment.NewLine, $"{Environment.NewLine}{Identation.Repeat(identationLevel)}")}";
